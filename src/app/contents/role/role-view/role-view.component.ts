...
export class RoleViewComponent implements OnInit {
    ...
    private loadRolePermissions(roleName: string) {
        ...
        this.roleService.getRolePermissions(roleName).subscribe(
            result => {
                ...
                const permissionsList: Array<Permission> = result.data;
                this.sessionPermissions.forEach(p => {
                    ...
                    if (webPer) {
                        ...
                        const per: Permission = permissionsList.find(pp => pp.code == p.code);
    ...
    showPermissionEditModal() {
        ...
        const permGroups: Array<PermissionGroup> = [];
        this.permissionGroups.forEach(g => {
            const group = new PermissionGroup(g.menuCode, g.menuName, []);
            if (g.permissions) {
